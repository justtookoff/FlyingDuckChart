...
BEGIN_MESSAGE_MAP(NSChartFormScrollBar, CWnd)
	ON_WM_CREATE()
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
	ON_WM_SIZE()
...
NSChartFormScrollBar::NSChartFormScrollBar(){
	this->nsChartForm = NULL;
}

NSChartFormScrollBar::~NSChartFormScrollBar(){
	if (this->nsChartForm != NULL){
		this->nsChartForm->OnClose();
		delete this->nsChartForm;
	}
}

int NSChartFormScrollBar::OnCreate(LPCREATESTRUCT lpCreateStruct) {
	...
	this->flyingDuckChart = (FlyingDuckChart*)CWnd::GetParent();

	////PrintPreview의 비트맵을 올려준다.
	this->nsChartForm = new NSChartForm;
	this->nsChartForm->Create(NULL, TEXT("NSChartForm"), WS_CHILD | WS_VISIBLE,
		CRect(0, 0, rect.Width() * 2, rect.Height() * 2.8), this, 100000);
...
void NSChartFormScrollBar::OnPaint(){
	PAINTSTRUCT Ps;
...
BOOL NSChartFormScrollBar::OnEraseBkgnd(CDC *pDC){
	return TRUE;
}

/////////////////////////////////////////스크롤/////////////////////////////////////////
void NSChartFormScrollBar::OnSize(UINT type, int x, int y){
...
void NSChartFormScrollBar::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar){
	...
	this->nsChartForm->Invalidate(TRUE);
	this->Invalidate();
	CWnd::OnVScroll(nSBCode, nPos, pScrollBar);
}

#if 0
void NSChartFormScrollBar::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar){
	// number of pixels to be scrolled
	Long delta;
...
BOOL NSChartFormScrollBar::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt){
	bool up = true;
	int delta = zDelta;
	if (zDelta < 0)
	{
...
void NSChartFormScrollBar::OnClose(){
